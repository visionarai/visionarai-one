...
import { ChoiceFormField, InputFormField } from "@visionarai-one/ui";
...
export function RenderConditionFields<T extends FieldValues>({ formControl, name, ...props }: RenderConditionFieldsProps<T>) {
	...
	return (
		<div className="flex gap-2 border-2 p-4 [&>*]:flex-1" {...props}>
			<ChoiceFormField
			...
			/>
			<ChoiceFormField formControl={formControl} label="Type of Value" name={`${name}.typeOfValue` as FieldPath<T>} options={ALL_TYPE_OF_VALUE_OPTIONS} />
			<ChoiceFormField formControl={formControl} label="Operation" name={`${name}.operation` as FieldPath<T>} options={operationOptions} />
