...
import { Condition, ConditionGroup, PolicyType, Subject } from './policy.zod.js';
...
const getSubjectFieldValue = (field: string, subject: Subject): unknown => {
  const path = field.replace(/^subject\./, '').split('.');
...
const evaluateCondition = (condition: Condition, subject: Subject): boolean => {
  ...
  if (!isSupportedType(value) || !isSupportedTypeOrArray(condition.value)) return false;
